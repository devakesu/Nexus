...
class ClientAIProfileState {
  final List<String> remotePaths; // Always exactly 5 elements corresponding to Slots 0-4
  final Map<int, File> pendingUploads;
  final Map<int, List<String>> slotSpecificVibeTags; // Tracks tags per individual photo slot
  final List<String> pendingDeletions; // Tracks remote storage paths queued for deletion
...
class ClientAIImageManager extends _$ClientAIImageManager {
  ...
  Future<void> stageImageSlot(int slotIndex, File localFile, {String userBranch = ''}) async {
    state = state.copyWith(isProcessingAI: true);

    // 1. Run your Local Client-Side AI Inference Core directly on-device
    final List<String> freshlyComputedTags = await _runOnDeviceVisionModel(localFile, userBranch: userBranch);
    ...
    if (slotIndex < state.remotePaths.length && state.remotePaths[slotIndex].isNotEmpty) {
      updatedDeletions.add(state.remotePaths[slotIndex]);
    }

    final updatedPending = Map<int, File>.from(state.pendingUploads).. [slotIndex] = localFile;
    final updatedTags = Map<int, List<String>>.from(state.slotSpecificVibeTags).. [slotIndex] = freshlyComputedTags;
  ...
  void clearImageSlot(int slotIndex) {
    ...
    if (slotIndex < updatedRemote.length) {
      ...
      updatedRemote[slotIndex] = ''; // Do NOT remove or shift elements, just empty the slot
    }
  ...
  Future<List<String>> _runOnDeviceVisionModel(File file, {String userBranch = ''}) async {
    final broker = RefinedEdgeVisionBroker();
    return broker.processImageToAestheticTags(imageFile: file, userBranch: userBranch);
  }
  ...
  Future<void> commitProfileChanges(Dio dioClient, String userId) async {
    ...
    if (state.pendingDeletions.isNotEmpty) {
      try {
        ...
        print('[ClientAIImageManager] Failed to remove old media files: $e');
      }
    }
    ...
    for (final entry in state.pendingUploads.entries) {
      ...
      final storagePath = '$userId/photo_${DateTime.now().millisecondsSinceEpoch}.jpg';
    ...
    final Set<String> unifiedUniqueTags = {};
    for (final tags in state.slotSpecificVibeTags.values) {
      unifiedUniqueTags.addAll(tags);
    }
    ...
    try {
      ...
      state = ClientAIProfileState(
        ...
        pendingUploads: {},
        slotSpecificVibeTags: state.slotSpecificVibeTags, // Retain for cache reference
        pendingDeletions: [],
        isSaving: false,
      );
    } catch (e) {
...
class RefinedEdgeVisionBroker {
  ...
  static const Set<String> _structuralNoiseBlacklist = {
    ...
    'ear', 'ears', 'chin', 'neck', 'sleeve', 'eyebrow', 'eyebrows', 'eyelash', 'eyelashes',
    'jaw', 'cheek', 'cheeks', 'lip', 'lips', 'mouth', 'forehead', 'hair', 'head', 'face',
    'facial-expression', 'smile', 'smiling', 'skin', 'teeth', 'tooth', 'human-leg', 'leg', 
    'legs', 'arm', 'arms', 'hand', 'hands', 'finger', 'fingers', 'joint', 'shoulder', 'shoulders',
    'human', 'person', 'people', 'sitting', 'standing', 'portrait', 'selfie',
    
    // Photographic & Metadata Artifacts
    'photography', 'snapshot', 'photo', 'image', 'picture', 'close-up', 'flash-photography', 
    'reflection', 'shadow', 'shadows', 'light', 'lighting', 'darkness', 'monochrome', 
    'black-and-white', 'lens-flare', 'blur', 'blurred', 'background', 'foreground',
    
    // Mundane Household & Spatial Infrastructure Clutter
    'wall', 'ceiling', 'floor', 'room', 'furniture', 'table', 'chair', 'window', 'door', 
    'building', 'house', 'roof', 'indoor', 'outdoor', 'inside', 'outside', 'daylighting',
    'property', 'architecture', 'surface', 'linens', 'textile', 'clothing', 'apparel'
  };
  ...
  Future<List<String>> processImageToAestheticTags({
    ...
    final File optimizedFile = await _downscaleInferenceTarget(imageFile);
    ...
    try {
      ...
      final List<String> compiledAestheticTags = [];

      for (var label in rawLabels) {
        // Base token normalization
        final String parsedLabel = label.label.toLowerCase().trim().replaceAll(' ', '-');
        ...
        if (_aestheticTranslationMatrix.containsKey(parsedLabel)) {
          ...
          if (targetTag == 'hacker-aesthetic' && userBranch.toUpperCase() == 'CSE') {
        ...
        if (parsedLabel.contains('music') || parsedLabel.contains('audio') || parsedLabel.contains('sound')) {
          compiledAestheticTags.add('analog-vibe');
          continue;
        }
        if (parsedLabel.contains('sport') || parsedLabel.contains('fitness') || parsedLabel.contains('athletic')) {
        ...
        final String sanitizedFallback = parsedLabel
      ...
      final List<String> uniquePayloadFeed = compiledAestheticTags.toSet().toList();
      ...
      print('[VisionBroker Critical] On-device labeling execution aborted: $e');
      return ['cosmic-dreamer'];
    } finally {
      await imageLabeler.close(); // Hard memory-leak prevention lock
      if (optimizedFile.path != imageFile.path && await optimizedFile.exists()) {
        await optimizedFile.delete(); // Delete localized temporary downscaled frames
      }
    }
  }
