...
class _SafetyScoreRingPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _SafetyScoreRingPainter oldDelegate) => oldDelegate.progress != progress;
}
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          child: Icon(flag.icon, color: const Color(0xFFEF4444), size: 18),
                        ),
                        const SizedBox(height: 12),
                        Text(
                          flag.title,
                          style: GoogleFonts.manrope(fontSize: 13.5, fontWeight: FontWeight.w800, color: const Color(0xFF0F172A)),
                        ),
          ...
          child: Row(
            ...
            children: List.generate(widget.flags.length, (i) {
              ...
              return AnimatedContainer(
                ...
                height: 6,
                decoration: BoxDecoration(
                  color: active ? const Color(0xFFEF4444) : const Color(0xFFFECACA),
...
class _SafetyCenterPageState extends State<SafetyCenterPage> {
  ...
  final List<Map<String, dynamic>> _quizQuestions = [
    {
      'question': 'A match asks you to send them money for an emergency before you meet. What is the safest course of action?',
      'options': [
      ...
      'correct': 1,
      'explanation': 'Never send money, share financial details, or invest in opportunities suggested by someone you met online. Genuine users will not ask you for money.',
    },
    {
      'question': 'When planning a first meeting, which location is the safest choice?',
      'options': [
      ...
      'correct': 2,
      'explanation': 'Always meet in public for the first few times. Do not go back to their home or private spaces until you know them well.',
    },
    {
      'question': 'Your match offers to pick you up at your house for your first date. How should you respond?',
      'options': [
      ...
      'correct': 0,
      'explanation': 'Always stay in control of your transportation. Drive yourself, take public transit, or use a rideshare service so you can leave whenever you feel uncomfortable.',
    },
  ];
  ...
  Future<void> _loadChecklistFlags() async {
    ...
    setState(() {
      ...
      _guidelinesReviewed = prefs.getBool('safety_guidelines_reviewed') ?? false;
    ...
    unawaited(
      showDialog<void>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          icon: const Icon(LucideIcons.shieldAlert, color: _red, size: 48)
              .animate(onPlay: (c) => c.repeat())
              .shake(duration: 800.ms, hz: 4),
          title: Text(
            'Emergency Triggered',
            style: GoogleFonts.manrope(fontWeight: FontWeight.w800, color: _red),
          ),
          content: Text(
            message,
  ...
  Future<void> _loadContacts() async {
    try {
      ...
      final loaded = list
          .map((item) => SafetyContact.fromJson(jsonDecode(item) as Map<String, dynamic>))
          .toList();
      setState(() {
        _contacts = loaded;
  ...
  Future<void> _saveContacts() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final list = _contacts.map((c) => jsonEncode(c.toJson())).toList();
      await prefs.setStringList('safety_contacts', list);
    } on Exception catch (_) {
      if (mounted) {
        NexusToast.show(context, 'Failed to save contacts', type: NexusToastType.error);
      }
    }
  }
  ...
  void _addContact(String name, String phone) {
    if (_contacts.length >= 3) {
      NexusToast.show(context, 'You can add a maximum of 3 trusted contacts.', type: NexusToastType.error);
      return;
    }
    setState(() {
    ...
    NexusToast.show(context, 'Contact added successfully', type: NexusToastType.success);
  }
  ...
  void _startCheckIn() {
    ...
    NexusToast.show(context, 'Check-in scheduled. Stay safe out there!', type: NexusToastType.success);
  }
  ...
  void _checkInSafely() {
    ...
    NexusToast.show(context, "Glad you're safe! Check-in closed.", type: NexusToastType.success);
  }
  ...
  void _extendCheckIn(Duration extra) {
    ...
    NexusToast.show(context, 'Added ${extra.inMinutes} minutes to your check-in.');
  }
  ...
  void _onCheckInExpired() {
    ...
    final contactName = (_checkInContactIndex != null && _checkInContactIndex! < _contacts.length)
    ...
    unawaited(
      showDialog<void>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          icon: const Icon(LucideIcons.alarmClockCheck, color: _red, size: 48)
              .animate(onPlay: (c) => c.repeat())
              .shake(duration: 800.ms, hz: 4),
          title: Text(
            'Check-In Missed',
  ...
  Future<void> _launchUrlHelper(String url) async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      if (mounted) {
        NexusToast.show(context, 'Could not launch helpline dialer', type: NexusToastType.error);
      }
    }
  }
        ...
        children: [
          ListView(
            ...
            children: [
              ...
              KeyedSubtree(key: _contactsKey, child: _buildEmergencyContactsSection()),
              _buildVerifiedProtectedSection(),
              _buildHelplinesSection(),
              _buildRedFlagsSection(),
              KeyedSubtree(key: _quizKey, child: _buildInteractiveQuizSection()),
              KeyedSubtree(key: _guidelinesKey, child: _buildSafetyEducationSection()),
  ...
  Widget _buildSafetyHeroBanner() {
    ...
    ).animate().fadeIn(duration: 350.ms, curve: Curves.easeOut).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
  ...
  List<Widget> _buildBreathingRings() {
    return List.generate(2, (i) {
      return Container(
        ...
        decoration: BoxDecoration(
          shape: BoxShape.circle,
          border: Border.all(color: _teal.withValues(alpha: 0.35), width: 1.5),
        ),
      )
          .animate(
            onPlay: (c) => c.repeat(),
  ...
  Widget _buildSafetyScoreSection() {
                  ...
                  return CustomPaint(
                    ...
                    child: SizedBox(
                      ...
                      child: Center(
                        child: Text(
                          '$percent%',
                          style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w900, color: const Color(0xFF0F172A)),
                        ),
                      ),
                    ),
                  );
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Your Safety Setup',
                      style: GoogleFonts.manrope(fontSize: 15, fontWeight: FontWeight.w800, color: const Color(0xFF0F172A)),
                    ),
                ...
                child: Row(
                  children: [
                    AnimatedContainer(
                      ...
                      height: 28,
                      decoration: BoxDecoration(
                        color: item.done ? _teal.withValues(alpha: 0.12) : const Color(0xFFF1F5F9),
                        shape: BoxShape.circle,
                      ),
                      child: Icon(
                    ...
                    Expanded(
                      child: Text(
                        ...
                        style: GoogleFonts.inter(
                          ...
                          color: item.done ? const Color(0xFF94A3B8) : const Color(0xFF334155),
    ...
    ).animate().fadeIn(delay: 30.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
  ...
  Widget _buildSosToolkitSection() {
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.shieldAlert, color: _red, size: 18),
              ),
              const SizedBox(width: 12),
              Text(
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            const Icon(LucideIcons.shieldAlert, color: Colors.white, size: 36),
                            const SizedBox(height: 6),
                            Text(
                              'ACTIVATE',
                              style: GoogleFonts.manrope(
    ...
    ).animate().fadeIn(delay: 50.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
    ...
    return List.generate(3, (i) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          .scale(begin: const Offset(1, 1), end: const Offset(1.45, 1.45), duration: 2100.ms, curve: Curves.easeOut)
          ...
          children: [
            Container(
              ...
              child: Center(
                ...
                .animate(onPlay: (c) => c.repeat())
                .scale(begin: const Offset(0.9, 0.9), end: const Offset(1.1, 1.1), duration: 500.ms, curve: Curves.easeInOut)
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.mapPin, color: _accent, size: 18),
              ),
              const SizedBox(width: 12),
              Text(
                'DATE CHECK-IN',
                style: GoogleFonts.manrope(fontSize: 12, fontWeight: FontWeight.w800, color: const Color(0xFF475569), letterSpacing: 1.2),
              ),
            ],
          ),
          const SizedBox(height: 12),
          ...
          AnimatedSwitcher(
            duration: 300.ms,
            child: _checkInActive ? _buildCheckInActiveCard() : _buildCheckInForm(),
          ),
        ],
      ),
    ).animate().fadeIn(delay: 60.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
  ...
  Widget _buildCheckInForm() {
    ...
    return Column(
      ...
      children: [
        Text(
          'PLAN (OPTIONAL)',
          style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w800, color: const Color(0xFF94A3B8), letterSpacing: 1),
        ),
        const SizedBox(height: 8),
        TextField(
          ...
          children: durationOptions.map((d) {
            ...
            return ScalePressable(
              ...
              child: Container(
                ...
                child: Text(
                  label,
                  style: GoogleFonts.manrope(fontSize: 12.5, fontWeight: FontWeight.w800, color: selected ? Colors.white : const Color(0xFF475569)),
                ),
              ),
            );
          }).toList(),
            ...
            children: List.generate(_contacts.length, (i) {
              ...
              return ScalePressable(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: selected ? _teal.withValues(alpha: 0.1) : const Color(0xFFF8FAFC),
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(color: selected ? _teal : const Color(0xFFE2E8F0)),
                  ),
                  child: Text(
                    _contacts[i].name,
                    style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w700, color: selected ? _teal : const Color(0xFF475569)),
                  ),
                ),
              );
            }),
        ...
        if (_contacts.isEmpty)
          Container(
            ...
            child: Row(
              children: [
                const Icon(LucideIcons.triangleAlert, color: Color(0xFFEA580C), size: 16),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
                    'Add a trusted contact first so we know who to alert.',
                    style: GoogleFonts.inter(fontSize: 12, color: const Color(0xFF9A3412), height: 1.35),
                  ),
                ),
              ],
            ),
            ...
            child: Container(
              ...
              child: Center(
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    const Icon(LucideIcons.navigation, size: 15, color: Colors.white),
  ...
  Widget _buildCheckInActiveCard() {
    final total = _checkInStartedDuration.inSeconds == 0 ? 1 : _checkInStartedDuration.inSeconds;
    final remainingFraction = (_checkInRemaining.inSeconds / total).clamp(0.0, 1.0);
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [_accent.withValues(alpha: 0.06), _teal.withValues(alpha: 0.06)],
            ),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: const Color(0xFFE2E8F0)),
          ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      timeText,
                      style: GoogleFonts.manrope(fontSize: 20, fontWeight: FontWeight.w900, color: const Color(0xFF0F172A)),
                    ),
                    Text(
        ...
        ScalePressable(
          ...
          child: Container(
            ...
            child: Center(
              child: Text(
                '+ Add 30 Minutes',
                style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w700, color: const Color(0xFF475569)),
              ),
            ),
          ),
        ),
        ...
        children: [
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(LucideIcons.users, color: _teal, size: 18),
                  ),
                  const SizedBox(width: 12),
                  Text(
                    'TRUSTED CONTACTS',
                ...
                ScalePressable(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        const Icon(LucideIcons.plus, size: 12, color: _accent),
          ...
          if (_loadingContacts)
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Icon(LucideIcons.userPlus, color: Color(0xFF94A3B8), size: 28),
                  const SizedBox(height: 8),
                  Text(
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(LucideIcons.user, color: _accent, size: 16),
                      ),
                      const SizedBox(width: 14),
                      Expanded(
                        child: Column(
                      ...
                      IconButton(
                        icon: const Icon(LucideIcons.trash2, color: Color(0xFF94A3B8), size: 18),
  ...
  Widget _buildVerifiedProtectedSection() {
    final badges = [
      (icon: LucideIcons.badgeCheck, title: 'Photo Verification', desc: 'The blue check confirms a profile’s photos were matched to a live selfie.'),
      (icon: LucideIcons.scanFace, title: 'Duplicate & Deepfake Scans', desc: 'Automated scans flag stolen, AI-generated, or duplicated profile photos.'),
      (icon: LucideIcons.lockKeyhole, title: 'Encrypted Sessions', desc: 'Your login session and personal data are encrypted at rest and in transit.'),
      (icon: LucideIcons.userRoundCheck, title: 'Identity Assurance', desc: 'Age and identity checks help keep the community authentic.'),
    ];
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.badgeCheck, color: _accent, size: 18),
              ),
              const SizedBox(width: 12),
              Text(
                'VERIFIED & PROTECTED',
          ...
          GridView.count(
            ...
            children: List.generate(badges.length, (i) {
              ...
              ).animate().fadeIn(delay: (i * 80).ms, duration: 300.ms).scale(begin: const Offset(0.94, 0.94), curve: Curves.easeOut);
            }),
          ),
        ],
      ),
    ).animate().fadeIn(delay: 120.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
  ...
  Widget _buildHelplinesSection() {
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.phoneCall, color: Color(0xFFEA580C), size: 18),
              ),
              const SizedBox(width: 12),
              Text(
                'CRISIS HELPLINES',
                    ...
                    ScalePressable(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(h['icon']! as IconData, size: 14, color: _accent),
                            const SizedBox(width: 8),
                            Text(
    ...
    ).animate().fadeIn(delay: 150.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }

  // --- Spot the Red Flags Section ---
  Widget _buildRedFlagsSection() {
    final flags = [
      (icon: LucideIcons.zap, title: 'Love Bombing', desc: 'Overwhelming affection, pet names, or talk of a future together within days of matching.'),
      (icon: LucideIcons.videoOff, title: 'Avoids Video Calls', desc: 'Always has an excuse to skip a video chat or phone call before meeting.'),
      (icon: LucideIcons.handCoins, title: 'Asks for Money', desc: 'Requests cash, gift cards, crypto, or your banking details — for any reason.'),
      (icon: LucideIcons.messageCircleOff, title: 'Rushes Off the App', desc: 'Pushes hard to move to texting or another app before you feel ready.'),
      (icon: LucideIcons.fileWarning, title: 'Inconsistent Story', desc: 'Details about their job, age, or life keep changing or don’t add up.'),
      (icon: LucideIcons.mapPinOff, title: 'Avoids Public Meetups', desc: 'Insists on meeting at a private home instead of a public place.'),
    ];
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(LucideIcons.triangleAlert, color: _red, size: 18),
                ),
                const SizedBox(width: 12),
                Text(
                  'SPOT THE RED FLAGS',
    ...
    ).animate().fadeIn(delay: 170.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
          ...
          children: List.generate(options.length, (optIdx) {
            ...
            if (showResults) {
              if (isCorrect) {
                ...
                borderColor = _teal;
                trailing = const Icon(LucideIcons.check, color: _teal, size: 16);
              } else if (isSelected) {
                tileBgColor = const Color(0xFFFEF2F2);
                borderColor = _red;
                trailing = const Icon(LucideIcons.alertCircle, color: _red, size: 16);
              }
            } else {
              if (isSelected) {
                borderColor = _accent;
              ...
              child: ScalePressable(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: tileBgColor,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: borderColor, width: isSelected || (showResults && isCorrect) ? 1.5 : 1),
                  ),
                  child: Row(
                    children: [
                      Expanded(
        ...
        if (_answered) ...[
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  _selectedAnswerIndex == correctIndex ? 'CORRECT' : 'INCORRECT',
                  style: GoogleFonts.manrope(
                    fontSize: 11,
                    fontWeight: FontWeight.w900,
          ...
          ScalePressable(
            ...
            child: Container(
              ...
              child: Center(
                child: Text(
                  _quizIndex < _quizQuestions.length ? 'Next Question' : 'View Results',
                  style: GoogleFonts.manrope(
                    fontSize: 14,
                    fontWeight: FontWeight.w800,
                    color: Colors.white,
  ...
  Widget _buildProtectionSection() {
    final items = [
      (icon: LucideIcons.bot, title: 'AI Photo & Message Moderation', desc: 'Automated systems scan for explicit content, spam, and scam patterns in real time.'),
      (icon: LucideIcons.headphones, title: '24/7 Human Trust & Safety Team', desc: 'Reports are reviewed around the clock by real people, every day of the year.'),
      (icon: LucideIcons.lockKeyhole, title: 'Encrypted Sessions & Data', desc: 'Your login session is encrypted using secure on-device storage.'),
      (icon: LucideIcons.ban, title: 'Instant Block, Report & Hide', desc: 'One tap removes someone from your matches, requests, and search results — permanently.'),
    ];
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [const Color(0xFF0F172A), const Color(0xFF0F172A).withValues(alpha: 0.92)],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        borderRadius: BorderRadius.circular(20),
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.shieldCheck, color: Color(0xFF5EEAD4), size: 18),
              ),
              const SizedBox(width: 12),
              Text(
                'HOW WE PROTECT YOU',
          ...
          ...List.generate(items.length, (i) {
            ...
            return Padding(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Icon(item.icon, color: const Color(0xFF5EEAD4), size: 18),
                  const SizedBox(width: 14),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
            ...
            ).animate().fadeIn(delay: (220 + i * 90).ms, duration: 300.ms).slideX(begin: 0.03, end: 0);
          }),
        ],
      ),
    ).animate().fadeIn(delay: 220.ms, duration: 350.ms).slideY(begin: 0.05, end: 0, duration: 350.ms);
  }
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.shield, color: _accent, size: 18),
              ),
              const SizedBox(width: 12),
              Text(
                'QUICK SHORTCUTS',
  ...
  Widget _buildFooterNote() {
    return Padding(
      ...
      child: Column(
        children: [
          const Icon(LucideIcons.heartHandshake, color: Color(0xFFCBD5E1), size: 22)
              .animate(onPlay: (c) => c.repeat(reverse: true))
              .scale(begin: const Offset(0.95, 0.95), end: const Offset(1.05, 1.05), duration: 2200.ms, curve: Curves.easeInOut),
          const SizedBox(height: 10),
          Text(
            "You're never alone here — help is always one tap away.",
            textAlign: TextAlign.center,
            style: GoogleFonts.manrope(fontSize: 12.5, fontWeight: FontWeight.w700, color: const Color(0xFF94A3B8)),
          ),
          const SizedBox(height: 4),
          Text(
