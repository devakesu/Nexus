...
async def _filter_unblocked_targets(user_id: str, candidate_ids: list[str]) -> list[str]:
    from app.db.discovery import get_cached_active_block_ids

    viewer_block_ids = await get_cached_active_block_ids(user_id)
    filtered = [tid for tid in candidate_ids if tid not in viewer_block_ids]
    if not filtered:
        return []

    target_blocks_list = await asyncio.gather(
        *[get_cached_active_block_ids(tid) for tid in filtered],
    )
    return [
        tid
        for tid, t_blocks in zip(filtered, target_blocks_list, strict=True)
        if user_id not in t_blocks
    ]


async def _fetch_presence_map(share_allowed_ids: list[str]) -> dict[str, dict[str, Any] | None]:
        ...
        for tid, raw in zip(share_allowed_ids, redis_results, strict=True):
            if raw:
                try:
                    ...
                    logger.debug("Malformed cached presence for user %s: %s", tid, err)
...
def _build_presence_response(
    p: dict[str, Any] | None, now: datetime,
) -> PresenceResponse | None:
    if not p:
        return None
...
async def _resolve_batch_presence(
    ...
    candidate_ids = [tid for tid in target_user_ids if tid in matched_target_ids]
    if not candidate_ids:
        return response_map

    # 2. Block checks from Redis cache
    unblocked_candidate_ids = await _filter_unblocked_targets(user_id, candidate_ids)
    ...
    presence_map = await _fetch_presence_map(share_allowed_ids)

    # 5. Build responses for share-allowed candidates
    now = utcnow()
    for tid in share_allowed_ids:
        resp = _build_presence_response(presence_map.get(tid), now)
        if resp is not None:
            response_map[tid] = resp
