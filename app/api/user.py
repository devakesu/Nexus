...
from app.db.users import fetch_public_user, get_supabase_user_from_jwt, is_allowed_college_email, update_user_terms, upsert_profile, upsert_public_user
...
@router.post("/api/v1/auth/complete-onboarding", response_model=CompleteOnboardingResponse)
@limiter.limit(settings.rate_limit_auth)
def complete_onboarding(
    request: Request,
    payload: CompleteOnboardingRequest = Body(...),
    _device: None = Depends(verify_app_check_token),
    access_token: str = Depends(get_bearer_token),
):
    _ = request 
